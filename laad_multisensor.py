...
#python laad_multi_sensor.py  --heart_rate COVID-19-Wearables/ASFODQR_hr.csv --steps COVID-19-Wearables/ASFODQR_steps.csv --sleep COVID-19-Wearables/ASFODQR_sleep.csv --myphd_id ASFODQR --symptom_date 2024-08-14
...
parser.add_argument('--sleep',metavar='', help ='raw sleep count with a header = sleep')
...
fitbit_oldProtocol_sleep = args.sleep
...
class RHRAD:
    ...
    def resting_heart_rate(self, heartrate, steps, sleep):
        """
        ...
        # sleep data
        df_sleep = pd.read_csv(fitbit_oldProtocol_sleep)

        # merge dataframes
        #df_hr = df_hr.resample('1min').median()
        #df_steps = df_steps.resample('1min').median()

        df1 = pd.merge(df_hr, df1_steps, left_index=True, right_index=True)
        df1 = df1.resample('1min').mean()
        df1 = df1.dropna()
        #day
        #df1 = df1.between_time('08:00','18:00')
        #night
        #df1 = df1.between_time('23:00','6:00')
        ...
        df1_rhr = df1.loc[(df1['steps_window_12'] == 0)]

        # steps
        df2_steps = df_steps[['datetime', 'steps']]
        df2_steps = df2_steps.set_index('datetime')
        df2_steps.index.name = None
        df2_steps.index = pd.to_datetime(df2_steps.index)
        #day
        #df2_steps = df2_steps.between_time('08:00','18:00')
        #night
        ...
        #day
        #df1_sleep = df1_sleep.between_time('08:00','18:00')
        #night
        #df1_sleep = df1_sleep.between_time('23:00','6:00')

        return df1_rhr, df2_steps, df1_sleep


    # pre-processing ------------------------------------------------------

    def pre_processing(self, df1_rhr, df2_steps, df1_sleep):
        """
        ...
        """
        # smooth data and  # resample

        df2_rhr = df1_rhr.dropna()
        df2_rhr = df2_rhr.rolling(400).mean()
        df2_rhr = df2_rhr.resample('24H').mean()
        df2_rhr = df2_rhr.drop(['steps'], axis=1)
        df2_rhr = df2_rhr.drop(['steps_window_12'], axis=1)
        df2_rhr = df2_rhr.dropna()
        df2_rhr = df2_rhr.rename(columns={"heartrate": "RHR"})

        df2_steps = df2_steps.rolling(400).mean()
        df2_steps = df2_steps.resample('24H').mean()

        df2_sleep = df1_sleep.rolling(400).mean()
        df2_sleep = df2_sleep.resample('24H').mean()

        df2 = df2_rhr.merge(df2_steps, left_index=True, right_index=True).merge(df2_sleep,left_index=True, right_index=True)
        processed_data = df2.ffill()
   ...
   # data splitting ------------------------------------------------------
    ...
    def standardization(self, df5_train, df5_valid, processed_data):
        """
        ...
        test_data[['steps']] = scaler.transform(test_data[['steps']])
        ...
        test_data[['sleep']] = scaler.transform(test_data[['sleep']])
        ...
        test_normal = test_normal.rename({'steps_y':'steps'}, axis=1)

        test_normal = test_normal.loc[test_normal['sleep_y'].isnull()]
        test_normal = test_normal.drop('sleep_y', axis=1)
        test_normal = test_normal.rename({'sleep_y':'sleep'}, axis=1)

        all_merged = pd.concat([df5_train, df5_valid, test])

        print(train.shape, df5_train.shape, df5_valid.shape, test.shape, test_normal.shape, test_anomaly.shape)

        return df5_train, df5_valid, test, test_data, test_normal, test_anomaly, all_merged
   ...
   # visualization ------------------------------------------------------
    ...
    def evaluate_complete_dataset(self, all_merged, THRESHOLD):
        ...
        ax1 = all_score_df.plot(figsize=(18,4))
        #plt.tight_layout()
        #plt.savefig(myphd_id + '_all_scores.pdf', bbox_inches='tight') 
        #print(all_anomalies)
...
# pre-process data
df1_rhr, df2_steps, df1_sleep = rhrad.resting_heart_rate(fitbit_oldProtocol_hr, fitbit_oldProtocol_steps, fitbit_oldProtocol_sleep)
processed_data = rhrad.pre_processing(df1_rhr, df2_steps, df1_sleep)

# split dates and data using assumptions listed in the paper
df5_train, df5_valid, symptom_date1, symptom_date_before_7, symptom_date_after_21 = rhrad.data_splitting(processed_data)

# standardization
train_data, valid_data, test, test_data, test_normal_data, test_anomaly_data, all_merged = rhrad.standardization(df5_train, df5_valid,processed_data)
...
model, history = rhrad.train_model(model, train_dataset, val_dataset, n_epochs=120)
